...
public interface MinecraftProvider {
     ...
     */
    default void sendChat(UserMessage message, ICommandSender scs) {
        scs.addChatMessage(messageToText(message));
    }

    /**
     * Utility class that implementing classes can use to perform a standard conversion of UserMessage to minecraft Text.
     */
    static IChatComponent messageToText(final UserMessage m) {
        final IChatComponent out;
        if (m.localized() != null) {
            out = new ChatComponentTranslation(m.localized().key(), m.localized().params());
        } else {
            out = new ChatComponentText(m.raw());
        }
        switch (m.style()) {
            case ERROR -> {
                out.setChatStyle(out.getChatStyle().setColor(EnumChatFormatting.RED));
            }
            case WARNING -> {
                out.setChatStyle(out.getChatStyle().setColor(EnumChatFormatting.YELLOW));
            }
            case JGIT -> {
                out.setChatStyle(out.getChatStyle().setColor(EnumChatFormatting.GRAY));
            }
            case NATIVE_GIT -> {
                out.setChatStyle(out.getChatStyle().setColor(EnumChatFormatting.GREEN));
            }
        }
        return out;
    }
